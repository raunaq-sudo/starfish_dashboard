...
class ComparatorTable extends Component {
  state = {
    locationMultiValue:
      this.props.locationValue[0] !== undefined
        ? this.props.locationValue[0]
        : '',
    data: [{ No_Data: '' }],
    type: 'cost_amt',
    name_type: 'Overview',
  ...
  fetchData = async () => {
    if (
      this.state.locationMultiValue.length !== 0 ||
                ...
                <Flex flex={1} justify={'end'}>
                  ...
                  <Dropdown title={this.state.name_type} size="sm">
                    ...
                    <Dropdown.Item
                      onClick={() => {
                        this.setState(
                          { type: 'budget_per', name_type: '% of budget' },
                          () => {
                            this.fetchData();
                          }
                        );
                ...
                {this.state.data !== undefined ? (
                  Array(this.state.data[0]).map(keys =>
                    Object.keys(keys).map(item => (
                      <Column
                        fixed={item == 'classification' || item == 'desc'}
                        flexGrow={1}
                        minWidth={200}
                      >
                        <HeaderCell>
